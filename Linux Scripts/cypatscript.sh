...
cont() {
    read -p "I have finished this task. Continue to the next task? (Y/N): " contyn
    if [[ "$contyn" = [Nn] ]]; then
        echo "Aborted"
        exit 0
    fi
...
PasswdFun() {
    echo -e "\033[1;31mChanging Root's Password..\033[0m"
    passwd
...
zeroUidFun() {
    echo -e "\033[1;31mChecking for 0 UID users...\033[0m"
    zerouidusers=$(awk -F: '$3 == 0 && $1 != "root" {print $1}' /etc/passwd)

    if [ -n "$zerouidusers" ]; then
        echo "There are Zero UID Users! Fixing it now..."
...
apacheSecFun() {
    echo -e "\033[1;31mSecuring Apache...\033[0m"
    # Secure Apache
